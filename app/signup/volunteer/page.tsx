...
export default function VolunteerSignupPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Validate Membership Code: BSPNWS_VOL_(last 4 digits of phone)
        const lastFourDigits = formData.phone.slice(-4);
        const expectedCode = `BSPNWS_VOL_${lastFourDigits}`;

        if (formData.membershipCode !== expectedCode) {
            setError('please put the valid code');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            // Save basic data for UI if needed (keeping original behavior mostly)
            const combinedData = {
                ...formData,
                id: data.user.id, // Store database ID for profile fetching
            ...
            localStorage.setItem('volunteer_data', JSON.stringify(combinedData));
                ...
                <div className="hidden lg:flex flex-1 relative group w-full max-w-[550px] aspect-square">
                    ...
                    <div className="relative w-full h-full rounded-[48px] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-[12px] border-white group-hover:scale-[1.01] transition-transform duration-500">
                        ...
                        <div className="absolute bottom-12 left-12 right-12 text-white">
                            <h2 className="text-4xl font-black leading-tight mb-4 drop-shadow-lg">Start Your Journey as a Hero</h2>
                            <p className="text-lg font-medium opacity-90 drop-shadow-md">Your contribution can create a ripple effect of kindness and change.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 w-full max-w-[500px]">
                    <div className="bg-white/80 backdrop-blur-3xl border border-white/60 rounded-[40px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] p-8 md:p-14 relative overflow-hidden group">
                        ...
                        <div className="relative z-10 text-center mb-8">
                            ...
                            <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">Join our community</p>
                            {error && (
                        ...
                        <form onSubmit={handleSubmit} className="relative z-10 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-gray-50/50 border border-transparent rounded-[20px] px-5 py-4 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-emerald-600/5 focus:bg-white focus:border-emerald-600/10 transition-all duration-300"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-black text-gray-500 uppercase tracking-widest ml-1">Membership Code</label>
                                    <input
                                        type="text"
                                        name="membershipCode"
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="px-2 pt-2">
                                    <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden flex gap-1">
                                        <div className={`h-full transition-all duration-500 ${formData.password.length > 0 ? 'bg-emerald-500' : 'bg-transparent'} ${formData.password.length > 4 ? 'w-1/3' : 'w-0'}`}></div>
                                        <div className={`h-full transition-all duration-500 ${formData.password.length > 6 ? 'bg-emerald-500' : 'bg-transparent'} ${formData.password.length > 6 ? 'w-1/3' : 'w-0'}`}></div>
                                        <div className={`h-full transition-all duration-500 ${formData.password.length > 8 ? 'bg-emerald-500' : 'bg-transparent'} ${formData.password.length > 8 ? 'w-1/4' : 'w-0'}`}></div>
                                    </div>
                                    <div className="flex justify-between items-center mt-2">
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Strong password</p>
                                        <span className={`text-[10px] font-black uppercase tracking-widest ${formData.password.length > 8 ? 'text-emerald-500' : 'text-gray-300'}`}>
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className={`w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-5 rounded-[20px] shadow-[0_12px_24px_-8px_rgba(5,150,105,0.3)] hover:shadow-[0_20px_40px_-12px_rgba(5,150,105,0.4)] transition-all duration-300 transform active:scale-[0.98] text-lg uppercase tracking-widest mt-4 flex items-center justify-center gap-3 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
                                {loading && (
